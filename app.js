document.addEventListener('DOMContentLoaded', async () => {
  ...
  const edCanvas = document.getElementById('edChart');
  const vaxCanvas = document.getElementById('vaxChart');
  const sepsisCanvas = document.getElementById('sepsisChart');

  if (!edCanvas || !vaxCanvas || !sepsisCanvas) return;

  const edCtx = edCanvas.getContext('2d');
  const vaxCtx = vaxCanvas.getContext('2d');
  const sepsisCtx = sepsisCanvas.getContext('2d');
